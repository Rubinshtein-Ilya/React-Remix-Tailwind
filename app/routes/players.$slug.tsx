import { useParams, useNavigate, Link } from "react-router";
import { useTranslation } from "react-i18next";
import { usePlayer, usePlayerItems, usePlayerPositions, useTeam } from "~/queries/public";
import { LikeButton } from "~/shared/buttons/LikeButton";
import ProductsCarousel from "~/shared/carousel/ProductsCarousel";
import { AppButton } from "~/shared/buttons/AppButton";
import { Spinner } from "~/shared/Spinner";
import { extractIdFromSlug } from "~/utils/slugUtils";
import useWindowWidth from "~/utils/useWindowWidth";

function PlayerPage() {
  const { slug } = useParams();
  const navigate = useNavigate();
  const { t } = useTranslation();
  const width = useWindowWidth();

  // Извлекаем ID из slug
  const playerId = extractIdFromSlug(slug || "");

  // Загружаем данные игрока через ID
  const { data: player, isLoading: playerLoading, error: playerError } = usePlayer(playerId);
  const { data: playerItems, isLoading: itemsLoading } = usePlayerItems(playerId);
  const { data: playerPositions } = usePlayerPositions();

  // Получаем teamId из player.team._id если команда есть
  const teamId = player?.team?._id || "";
  const { data: playerTeam, isLoading: teamLoading } = useTeam(teamId);

  const allItems = playerItems?.pages.flatMap((page) => page.items) || [];

  // Показываем состояние загрузки
  if (playerLoading) {
    return (
      <div className="flex flex-col gap-4 mx-auto py-8 sm:py-12 lg:py-16 mt-15 bg-[var(--bg-gray)]">
        <div className="container px-4 pb-8 sm:pb-12 lg:pb-21">
          <div className="text-center py-12 sm:py-16 lg:py-20">
            <Spinner size="lg" text={t("pages.players.loading")} />
          </div>
        </div>
      </div>
    );
  }

  // Показываем ошибку
  if (playerError || !player) {
    return (
      <div className="flex flex-col gap-4 mx-auto py-8 sm:py-12 lg:py-16 mt-15 bg-[var(--bg-gray)]">
        <div className="container px-4 pb-8 sm:pb-12 lg:pb-21">
          <div className="text-center py-12 sm:py-16 lg:py-20">
            <div className="text-xl sm:text-2xl text-red-500">{t("pages.players.not_found")}</div>
            <AppButton
              variant="primary"
              className="mt-4 max-w-xs mx-auto"
              onClick={() => navigate("/")}
            >
              {t("common.back", "Назад")}
            </AppButton>
          </div>
        </div>
      </div>
    );
  }

  const translatedCountry = t(`country.${player.birthPlaceCountry || ""}`)

  // Формируем место рождения из существующих полей
  const birthPlace = player.birthPlaceCity
    ? `${player.birthPlaceCity}, ${translatedCountry}`
    : translatedCountry;

  return (
    <div className="flex flex-col gap-4 mx-auto py-8 sm:py-12 lg:py-14 mt-14">
      <div className="w-full px-10 lg:h-120 bg-[var(--bg-gray)]">
        <div className="flex flex-col lg:h-120 mx-auto lg:max-w-[1195px] lg:flex-row lg:justify-between 2xl:justify-start gap-25.75">
          <div className="h-full self-center relative mx-auto sm:min-w-113.5 w-full xl:w-113.5 flex-shrink-0 flex flex-col justify-center gap-y-4.5 sm:gap-y-5.25 text-[var(--text-primary)]">
            <div className="xl:hidden w-full h-60 flex-shrink-0">
              <img
                className="w-full h-full object-contain"
                src={
                  player.images[0] ||
                  "https://fansdream.ru/assets/logo-Bho2Piyd.svg"
                }
                alt="banner image"
              />
            </div>

            <div className="flex flex-col justify-center gap-y-2 xl:gap-y-4.5 text-[var(--text-primary)] px-4 pt-0 pb-4 xl:px-0">
              <div className="gap-4 flex items-center">
                <LikeButton
                  sourceId={player._id}
                  type="player"
                  size={width && width < 640 ? "sm" : "lg"}
                  isLiked={player.isLiked}
                />
                <h1 className="text-[22px] lg:text-[30px] font-medium uppercase">
                  {player.name} {player.lastName}
                </h1>
              </div>
              {player.description && (
                <p className="text-[14px] lg:text-[16px] text-pretty">
                  {player.description}
                </p>
              )}

              {!player.description && (
                <>
                  <div className="flex sm:items-center gap-1 sm:gap-2">
                    <span className="font-medium text-sm sm:text-base">
                      {t("pages.players.info.position")}:
                    </span>
                    <span className="text-[var(--text-primary)] text-sm sm:text-base">
                      {
                        playerPositions?.[player.sportId]?.find(
                          (position) => position._id === player.position
                        )?.name
                      }
                    </span>
                  </div>

                  {player.number && (
                    <div className="flex  sm:items-center gap-1 sm:gap-2">
                      <span className="font-medium text-sm sm:text-base">
                        {t("pages.players.info.number")}:
                      </span>
                      <span className="text-[var(--text-primary)] text-sm sm:text-base">
                        #{player.number}
                      </span>
                    </div>
                  )}

                  <div className="flex  sm:items-center gap-1 sm:gap-2">
                    <span className="font-medium text-sm sm:text-base">
                      {t("pages.players.info.birth_date")}:
                    </span>
                    <span className="text-[var(--text-primary)] text-sm sm:text-base">
                      {new Date(player.birthDate).toLocaleDateString()}
                    </span>
                  </div>

                  <div className="flex  sm:items-center gap-1 sm:gap-2">
                    <span className="font-medium text-sm sm:text-base">
                      {t("pages.players.info.birth_place")}:
                    </span>
                    <span className="text-[var(--text-primary)] text-sm sm:text-base">
                      {birthPlace}
                    </span>
                  </div>
                </>
              )}
            </div>
          </div>
          <div
            className="shrink-0 hidden xl:block"
            style={{ minWidth: "30vw" }}
          >
            <img
              className="w-full h-full object-contain"
              src={
                player.images[0] ||
                "https://fansdream.ru/assets/logo-Bho2Piyd.svg"
              }
              alt="banner image"
            />
          </div>
        </div>
      </div>

      <div className="container px-4 pb-8 sm:pb-12 lg:pb-21">
        {/* Player info section */}
        {/* Items section */}
        <section className="player-items">
          {itemsLoading ? (
            <div className="flex justify-center py-12 sm:py-16">
              <Spinner size="lg" text={t("pages.players.items.loading")} />
            </div>
          ) : allItems.length > 0 ? (
            <div className="overflow-hidden">
              <ProductsCarousel
                items={allItems}
                title={t("pages.players.items.items_on_sale")}
                heading="title"
                slideWidth="18rem"
              />
            </div>
          ) : (
            <div className="text-center py-12 sm:py-16 lg:py-24 bg-[var(--bg-primary)] rounded-lg border border-[var(--border-muted)]">
              <div className="text-xl sm:text-2xl text-[var(--text-primary)] mb-3 sm:mb-4">
                {t("pages.players.items.no_items")}
              </div>
              <div className="text-sm sm:text-base text-[var(--text-primary)] px-4">
                {t(
                  "pages.players.items.no_items_description",
                  "Товары этого игрока скоро появятся в нашем каталоге"
                )}
              </div>
            </div>
          )}
        </section>
      </div>
    </div>
  );
}

export default PlayerPage;
